...
class FacturacionModerna {
    ...
    public function __construct( $url, $opciones = array(), $debug = 0 ) {
        $this->url = $url;
        $this->debug = $debug;
        //$this->log = Config::getInstance()->get("path_logfiles") . '/facturacionmoderna_log.log';
        ...
        if ( $debug ) {
            //Logger::getLogger()->debug($this->url);
            Logger($this)->debug($this->url);
        }
    }
    ...
    public function timbrar( $str, $opciones = array('generarCBB' => false, 'generarTXT' => false, 'generarPDF' => false) ) {
        try {
            ...
            if ( $this->debug == 1 ) {
                Logger::getLogger()->debug("SOAP request:\t" . $cliente->__getLastRequest());
                Logger::getLogger()->debug("SOAP response:\t" . $cliente->__getLastResponse());
            }
    ...
    public function cancelar( $uuid ) {
        try {
            ...
            if ( strpos($e->getMessage(), 'l comprobante aun se encuentra en proceso de envio') === false && strpos($e->getMessage(), 'l UUID se encuentra en proceso de cancelac') === false ) {
                Logger($this)->error($e);
            }
        } catch ( \Exception $e ) {
            $this->ultimoError = $e->getMessage();
    ...
    private function log( $str ) {
        ...
        if ( !$fd ) {
            ...
            if ( !fwrite($fd, "[ " . date('Y-m-d H:i:s') . " ] " . $str . "\n") ) {
                echo "<pre> $data </pre>";
            }
